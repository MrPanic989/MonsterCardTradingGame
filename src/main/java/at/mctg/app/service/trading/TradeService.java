...
public class TradeService implements Service {
    ...
    public Response handleRequest(Request request) {
        ...
        } else if (request.getMethod() == Method.DELETE && parts.size() == 2) {

            return this.tradeController.deleteTrade(request, parts.get(1));

        } else if (request.getMethod() == Method.POST &&
                request.getPathParts().size() > 1) {

            return this.tradeController.executeTrade(request, request.getPathParts().get(1));
        }
            return new Response(
